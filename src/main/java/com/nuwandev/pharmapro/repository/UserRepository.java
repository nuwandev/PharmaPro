...
public class UserRepository {
    ...
    public Optional<User> findByUsername(String username) {
        List<User> users =
                jdbcTemplate.query(
                        """
                                SELECT id, username, full_name, email, password_hash, role, status, created_at, updated_at, remember_me_token
                                FROM users
                                WHERE username = ?
                                LIMIT 1
                                """,
    ...
    public void updateRememberMeToken(Long userId, String token) {
        jdbcTemplate.update("UPDATE users SET remember_me_token = ? WHERE id = ?", token, userId);
    }
}
